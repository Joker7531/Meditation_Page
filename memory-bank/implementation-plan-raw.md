...
- P0：渲染 40–60 个光点，80% 在主圈附近，20% 外围散落；颜色渐变与发光接近 spec。
- 先不做漂移/闪烁也可（那是 P1），但结构要支持后续加。

**测试**
- RTL：断言光点元素数量在 [40, 60] 范围内（通过 data-attribute 或可查询结构）。
...
- 记录一次构建产物体积与首屏加载（本地/预览环境即可）。
- 若超标：优先处理字体加载闪烁（回退字体）、减少不必要资源。
